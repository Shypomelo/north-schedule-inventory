...
export const workbenchAdapter={
 ...
 async promote(todoId:string,zoneId:string,projectId:string|null,start:string|null,due:string|null) {const {error}=await supabase.rpc('promote_private_todo_to_work_item',{p_todo_id:todoId,p_work_zone_id:zoneId,p_project_id:projectId,p_expected_start_date:start,p_due_date:due});if(error)throw error;},
 async updateItem(item:WorkItem,fields:Pick<WorkItem,'title'|'content'|'project_id'|'received_at'|'expected_start_date'|'due_date'|'status'>) {
