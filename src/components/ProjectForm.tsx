...
export function ProjectForm({ initialData, onSubmit, onCancel, isSubmitting }: ProjectFormProps) {
  ...
  const InputField = ({ label, field, required = false, type = 'text', colSpan = 1 }: { label: string, field: keyof typeof formData, required?: boolean, type?: string, colSpan?: number }) => (
    <label className={`flex flex-col gap-1 ${colSpan === 2 ? 'md:col-span-2' : ''} ${colSpan === 3 ? 'md:col-span-3' : ''}`}>
      <span className="text-sm font-semibold text-secondary">{label} {required && <span className="text-danger">*</span>}</span>
      <input 
        type={type} required={required}
        className="bg-page border border-theme-border rounded p-2.5 focus:border-accent outline-none text-primary placeholder:text-secondary/50 transition-colors w-full"
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col h-full w-full max-h-[85vh]">
      ...
      {/* Tabs Header */}
      <div className="flex gap-2 border-b border-theme-border/50 mb-6 overflow-x-auto shrink-0 hide-scrollbar pb-1">
        <button 
          type="button" 
          onClick={() => setActiveTab(1)} 
          className={`px-4 py-2.5 rounded-t-lg font-medium whitespace-nowrap transition-colors ${activeTab === 1 ? 'bg-card border-t-2 border-accent text-accent shadow-sm' : 'border-t-2 border-transparent text-secondary hover:text-primary hover:bg-card/50'}`}
        >
          基本資料
        </button>
        <button 
          ...
          className={`px-4 py-2.5 rounded-t-lg font-medium whitespace-nowrap transition-colors ${activeTab === 2 ? 'bg-card border-t-2 border-accent text-accent shadow-sm' : 'border-t-2 border-transparent text-secondary hover:text-primary hover:bg-card/50'}`}
        >
          聯絡資料
        </button>
        <button 
          ...
          className={`px-4 py-2.5 rounded-t-lg font-medium whitespace-nowrap transition-colors ${activeTab === 3 ? 'bg-card border-t-2 border-accent text-accent shadow-sm' : 'border-t-2 border-transparent text-secondary hover:text-primary hover:bg-card/50'}`}
        >
          保固 / 維運
        </button>
        <button 
          ...
          className={`px-4 py-2.5 rounded-t-lg font-medium whitespace-nowrap transition-colors ${activeTab === 4 ? 'bg-card border-t-2 border-accent text-accent shadow-sm' : 'border-t-2 border-transparent text-secondary hover:text-primary hover:bg-card/50'}`}
        >
          設備 / 備註
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar min-h-[300px]">
        ...
        {activeTab === 1 && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 animate-in fade-in duration-300">
            ...
            <label className="flex flex-col gap-1">
              <span className="text-sm font-semibold text-secondary">案場狀態</span>
              <select 
            ...
            <label className="flex items-center gap-3 mt-2 md:col-span-3 bg-card/40 p-3 rounded-lg border border-theme-border/50">
              <input 
                type="checkbox" 
                className="w-5 h-5 accent-accent cursor-pointer rounded border-theme-border"
                checked={formData.is_active} 
                onChange={e => setFormData({...formData, is_active: e.target.checked})} 
              />
              <span className="text-sm font-medium text-secondary select-none cursor-pointer">是否啟用此案場 (停用將不出現在選單)</span>
            </label>
          </div>
        )}
        ...
        {activeTab === 3 && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 animate-in fade-in duration-300">
            ...
            <div className="md:col-span-2 lg:col-span-3 grid grid-cols-1 md:grid-cols-2 gap-5 pt-2 border-t border-theme-border/50 mt-2">
              ...
              <label className="flex flex-col gap-1 md:col-span-2">
                <span className="text-sm font-semibold text-secondary">維保說明</span>
                <textarea 
                  className="bg-page border border-theme-border rounded p-2.5 focus:border-accent outline-none min-h-[100px] text-primary transition-colors w-full"
        ...
        {activeTab === 4 && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 animate-in fade-in duration-300">
            ...
            <label className="flex flex-col gap-1 md:col-span-2 lg:col-span-3">
              <span className="text-sm font-semibold text-secondary">備註 (Notes)</span>
              <textarea 
                className="bg-page border border-theme-border rounded p-2.5 focus:border-accent outline-none min-h-[100px] text-primary transition-colors w-full"
            ...
            <div className="md:col-span-2 lg:col-span-3 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 pt-4 border-t border-theme-border/50 mt-2 opacity-70">
              <div className="lg:col-span-4 text-accent font-medium text-sm">巡檢提醒設定 (預留功能)</div>
      ...
      {/* Footer / Actions */}
      <div className="flex justify-end gap-4 mt-6 pt-5 border-t border-theme-border shrink-0">
        <button type="button" onClick={onCancel} disabled={isSubmitting} className="px-4 py-2 rounded-lg bg-card hover:bg-page border border-theme-border text-secondary hover:text-primary transition disabled:opacity-50 font-medium">
          取消
        </button>
        <button type="submit" disabled={isSubmitting || isViewer} className="px-4 py-2 rounded-lg bg-accent hover:bg-accent-hover text-white font-semibold disabled:opacity-50 shadow-lg shadow-accent/20 transition">
