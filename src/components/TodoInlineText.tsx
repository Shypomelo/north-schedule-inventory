...
export function TodoInlineText({todo,onSaved}:{todo:Todo;onSaved:()=>Promise<void>}) {
 ...
 if(!editing)return <button type="button" disabled={!editable} onClick={open} aria-label={`編輯待辦文字：${todo.title}`} className="block min-h-11 w-full min-w-0 text-left disabled:cursor-default"><span className="block break-words font-medium">{todo.title}</span>{todo.content&&<span className="mt-1 block whitespace-pre-wrap break-words text-sm text-secondary">{todo.content}</span>}{todo.status==='已收納'&&<span className="text-xs text-secondary">已收納</span>}</button>;
