...
1. **Login / manual UI review remains independent:** automated Google OAuth is blocked by the browser provider and localhost Login remains an external HOLD. The candidate membership resolver and RLS edge case are covered by local regression tests; no Production migration was applied.
