-- Business conflicts must not use serialization_failure: PostgREST 14 retries it.
-- PT409 returns one HTTP conflict response and requires operator confirmation.
...
BEGIN
  ...
  RETURN to_jsonb(tx);
END $$;
CREATE OR REPLACE FUNCTION public.register_inventory_serial_atomic(p_serial_no text,p_link_id uuid DEFAULT NULL,p_batch_id uuid DEFAULT NULL)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path='' AS $$
#variable_conflict use_variable
DECLARE
  a public.team_members := app_private.inventory_actor();
...
BEGIN
  ...
  IF tx_id IS NOT NULL THEN
    ...
    IF tx.item_id IS DISTINCT FROM item_id THEN RAISE EXCEPTION 'STALE_INVENTORY' USING ERRCODE='PT409'; END IF;
